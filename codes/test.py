...
def pred_test_samples():
    ...
    for mm, row in enumerate(case_slices):
        ...
        ax.imshow(X_test[n_imgs[row[0]-1]+row[1],:,:,0], cmap='gray' )

        contours = find_contours(y_pred[n_imgs[row[0]-1]+row[1],:,:,0], 0.01, fully_connected='high')
        for n, contour in enumerate(contours):
            ax.plot(contour[:, 1], contour[:, 0], linewidth=1, color='b')
...
def check_predictions(the_list, plot=False ):
    ...
    start_ind = 0
    end_ind  = 0
    for y_true in read_cases(the_list, fileList):
